...
def test_load_key_from_1248_vertical_loads_words(m5stickv, mocker, tdata):
    ...
    # Reuse the exact entry sequence from test_enter_stackbit_vertical:
    ABANDON_SEQ = (
        [BUTTON_PAGE] * 3
        + [BUTTON_ENTER]
        + [BUTTON_PAGE_PREV] * 4
...
def test_enter_stackbit_vertical_navigation_skips_invalid_cells(m5stickv, mocker):
    ...
    # The two skip transitions actually fired in both directions:
    forward_returns = [
...
def test_enter_stackbit_vertical_clamps_digit_on_overflow(m5stickv, mocker):
    ...
    # _toggle_bit_vertical mutates digits in place, so a plain spy would record
    # four references to the same list; wrap to snapshot real state per call.
    snapshots = []
